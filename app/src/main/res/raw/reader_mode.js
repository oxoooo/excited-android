...
try {
    if (ReaderArticleFinderJS.isReaderModeAvailable()) {
        ...
        var article = ReaderArticleFinderJS.article.element;
        ...
        // clean all empty paragraphs
        article.querySelectorAll('p').forEach((node) => {
            if (!node.innerText.trim()) {
                node.remove();
            }
        });

        ReaderMode.enter(title, article.innerHTML);
    } else {
        console.log('reader mode not available');
        ReaderMode.unavailable();
    }
} catch (e) {
    console.log('page not loaded completely, retry later...');
